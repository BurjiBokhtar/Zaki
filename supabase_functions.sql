...
-- ───────────────────────────────────────
-- Долг → касса: при выдаче/получении долга и при его возврате приложение
-- уже пишет операцию в operations. Раньше эта операция ничем не была
-- связана с долгом, поэтому при удалении долга деньги «зависали» в кассе
-- навсегда. Теперь операция помечается debt_id, и удаление долга находит
-- все свои операции и добавляет им зеркальную противоположность —
-- деньги возвращаются на баланс, а история не стирается.
alter table operations add column if not exists debt_id int8 references debts(id) on delete set null;
notify pgrst, 'reload schema';
-- ───────────────────────────────────────
